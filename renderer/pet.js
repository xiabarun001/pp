...
function refreshClasses() {
  wrap.className = `pet-wrap state-${state}`
    + (hasFrames() ? ' skin-img' : '')
    + (motionMode === 'walk' || motionMode === 'return' ? ' moving-walk' : '')
...
function dismiss() {
  ...
  if (notifyQueue.length) {
    revertTimer = setTimeout(() => displayNotify(notifyQueue.shift()), 350); // 下一条排队消息
  } else {
...
bubble.addEventListener('click', () => {
  if (currentUrl) window.pet.openUrl(currentUrl); // 带链接的气泡:点击先开链接
  dismiss();
});

// 主进程要求收气泡(调试端点 /dismiss 用)
window.pet.onDismiss?.(() => dismiss());

// ── 收通知:sticky 消息在展示时,后到的通知排队,点掉一条露出下一条 ──
const notifyQueue = [];
let currentUrl = null;

function displayNotify(p) {
  markActivity();
  currentUrl = p.url || null;
  bubble.classList.toggle('has-url', !!currentUrl);
...
window.pet.onNotify((p) => {
  if (sticky && bubble.classList.contains('show')) {
    notifyQueue.push(p);
    if (notifyQueue.length > 10) notifyQueue.shift(); // 上限保护,挤掉最旧的
    return;
  }
  displayNotify(p);
});
